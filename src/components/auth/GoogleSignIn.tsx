  ...
  const handleTokenResponse = useCallback(async (response: { access_token?: string; error?: string }) => {
    ...
    if (response.access_token) {
      try {
        ...
        // Create a mock ID token with user info (for compatibility with OAuth2 flow)
        const mockIdToken = btoa(JSON.stringify({
          ...
          aud: response.access_token, // Store access token for reference
        }));
  ...
  const handleCredentialResponse = useCallback((response: { credential?: string }) => {
    try {
      if (response.credential) {
        // Pass the Google ID token directly to parent component
