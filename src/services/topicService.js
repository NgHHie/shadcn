...
export const createTopic = async(payload) => {
    const  { data } =  await axiosInstance.post(`/api/topics`,payload);
    return data;
}
...
export const getTopicDetail = async(topicId) => {
    const  { data } =  await axiosInstance.get(`/api/topics/${topicId}`);
    return data;
}

export const getNumberTopicNew = async() => {
    const  { data } =  await axiosInstance.get(`/api/topics/count-new`);
    return data;
}